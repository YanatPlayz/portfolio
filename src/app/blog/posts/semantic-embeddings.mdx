...
There's also different models that can be used to generate embeddings. Word2vec, mentioned earlier, is a key example. There's also [fastText](https://fasttext.cc/), which is an extension of Word2vec that uses character n-grams (e.g. 'apple' becomes [ap, app, ppl, ple, le]) to represent words. It basically operates at the sub-word level, which can be helpful for languages like Sanskrit that are morphologically rich [^6].

I'm currently in the process of training models with different configurations to evaluate which techniques are most effective for Sanskrit. I hope to publish my results in the near future - stay tuned for that. Currently, if you're interested, check out [my other blog posts](https://tanayagrawal.xyz/blog/) for more details about this project or me.
